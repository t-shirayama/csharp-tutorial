...
- 注意: attribute を付ければ自動で処理されると思い込む。
- 注意: 自作 attribute を作ったが、それを読む処理がどこにもない。
- 注意: reflection で property 名を文字列指定し、rename に弱い実装にする。
- 注意: 通常の method 呼び出しで十分な場所に reflection を使う。
- 注意: AOT / trimming を使う環境で、reflection 前提の code が動くか確認しない。
